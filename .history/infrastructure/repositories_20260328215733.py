...
def get_weekly_income(self):
        """Ingresos de los últimos 7 días con comparativo."""
        sql = f"""
            SELECT 
                TO_CHAR({self.PAYMENT_DATE_FIELD}, 'ID') as dia_semana,
                TO_CHAR({self.PAYMENT_DATE_FIELD}, 'Day') as nombre_dia,
                SUM({self.AMOUNT_FIELD}) as total
            FROM {self.PAYMENT_TABLE}
            WHERE {self.PAYMENT_DATE_FIELD} >= CURRENT_DATE - INTERVAL '7 days'
            GROUP BY dia_semana, nombre_dia
            ORDER BY dia_semana
        """
        with connection.cursor() as cursor:
            cursor.execute(sql)
            return [{"dia": r[1].strip(), "total": float(r[2])} for r in rows]

    def get_stay_metrics(self):
        """Calcula el tiempo promedio que los vehículos pasan dentro (en minutos)."""
        # Solo para tickets cerrados (con exit_time)
        sql = f"""
            SELECT 
                AVG(EXTRACT(EPOCH FROM (exit_time - entry_time))/60) as promedio_minutos,
                MAX(EXTRACT(EPOCH FROM (exit_time - entry_time))/60) as max_minutos
