...
def reports_view(request):
    ...
    active_tickets = usage_stats.get("active", 0)
    
    occupancy_rate = 0
    if total_tickets > 0:
        occupancy_rate = round((active_tickets / total_tickets) * 100, 2)

    average_ticket = 0
    daily_count = daily_income.get("ticket_count", 0)
    if daily_count > 0:
        average_ticket = daily_income["total"] / daily_count
