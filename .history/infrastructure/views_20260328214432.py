...
from infrastructure.repositories.django_report_repository import DjangoReportRepository
...
def reports_view(request):
    ...
    daily_income = repo.get_daily_income(today)
    monthly_income = repo.get_monthly_income(today.year, today.month)
    vehicle_stats = repo.get_vehicle_type_stats()
    frequent_clients = repo.get_frequent_clients()
    usage_stats = repo.get_usage_stats()

    occupancy_rate = 0
    total_tickets = usage_stats.get("total_tickets", 0)

    if total_tickets > 0:
        active = usage_stats.get("active", 0)
        occupancy_rate = round((active / total_tickets) * 100, 2)

    average_ticket = 0
    if daily_income.get("ticket_count", 0) > 0:
        average_ticket = daily_income["total"] / daily_income["ticket_count"]
