...
class DjangoReportRepository:
    ...
    def get_revenue_by_day_of_week(self):
        ...
        stats = Ticket.objects.filter(
            ...
            dia_num=ExtractWeekDay('exit_time')
        ).values('dia_num').annotate(
            total=Sum('total_paid')
        ).order_by('dia_num')

        return [
            {"nombre": dias_nombres.get(s['dia_num']), "total": float(s['total'] or 0)} 
            for s in stats
        ]

    def get_stay_metrics(self):
        """Calcula tiempos de estadía en minutos"""
        # Extraemos la diferencia en segundos y dividimos por 60
        tickets = Ticket.objects.filter(status="CLOSED", exit_time__isnull=False)
        
        # Lógica simple con Python para evitar líos de SQL con duraciones
        duraciones = []
        for t in tickets:
            if t.exit_time and t.entry_time:
                diff = (t.exit_time - t.entry_time).total_seconds() / 60
                duraciones.append(diff)
    ...
    def get_vehicle_type_stats(self):
        return Ticket.objects.values('vehicle__type').annotate(
            total_services=Count('id')
        ).order_by('-total_services')

    def get_frequent_clients(self):
        return Ticket.objects.values('vehicle__client__name', 'vehicle__client__phone').annotate(
            visit_count=Count('id')
        ).order_by('-visit_count')[:5]

    def get_monthly_income(self):
        """Historial por día para la tabla"""
        return Ticket.objects.filter(
            status="CLOSED",
            exit_time__month=timezone.now().month
        ).annotate(
            day=TruncDay('exit_time')
        ).values('day').annotate(
            total=Sum('total_paid'),
