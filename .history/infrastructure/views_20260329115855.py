...
def delete_client_view(request, id):
    ...
    if request.method == 'POST':
        # Verificamos si este cliente tiene algún ticket activo en el parqueadero
        # Ajusta 'client' y 'fecha_salida' según los nombres de tus campos
        ...
        if has_active_ticket:
            # Si tiene un vehículo adentro, bloqueamos la eliminación
            messages.error(
                request, 
                f"No se puede eliminar a {client_obj.name} porque tiene un vehículo con un ticket activo en el sistema."
            )
            return redirect('/clientes/')

        # Si todo está en orden, procedemos a borrar
...
def delete_vehicle_view(request, id):
    ...
    if request.method == 'POST':
        # Buscamos si hay tickets activos (donde la fecha/hora de salida sea None)
        # Ajusta 'fecha_salida' al nombre real de tu campo en el modelo Ticket
        has_active_ticket = Ticket.objects.filter(vehicle=vehicle_obj, fecha_salida__isnull=True).exists()

        if has_active_ticket:
            # Si tiene ticket activo, enviamos un mensaje de error y no borramos nada
            messages.error(request, f"No se puede eliminar el vehículo {vehicle_obj.plate} porque tiene un ticket activo en el parqueadero.")
            return redirect('/vehiculos/')
        
        # Si no hay tickets activos, procedemos a borrar
        vehicle_obj.delete()
