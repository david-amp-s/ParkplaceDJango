...
from .models import ParkingSpot, Payment as PaymentModel  # Asegúrate de que el nombre coincida con tu models.py
...
# REPOSITORIO DE REPORTES (Corregido)
class DjangoReportRepository(ReportRepositoryPort):
    # Ajusta estos nombres si tus tablas en DB son diferentes
    PAYMENT_TABLE = "infrastructure_payment" # Por defecto Django usa app_modelo
    ...
    def get_daily_income(self, target_date=None):
        ...
        sql = f"""
            ...
            WHERE {self.PAYMENT_DATE_FIELD}::date = %s
        """
    ...
    def get_vehicle_type_stats(self):
        # Asegúrate que los nombres de tablas coincidan: infrastructure_ticket, etc.
        sql = """
            SELECT v.type, COUNT(t.id)
            FROM infrastructure_ticket t
            JOIN infrastructure_vehicle v ON v.id = t.vehicle_id
            GROUP BY v.type
        ...
        with connection.cursor() as cursor:
            cursor.execute(sql)
            rows = cursor.fetchall()
        return [{"vehicle_type": r[0], "total_services": r[1]} for r in rows]

    def get_frequent_clients(self, limit=10):
        sql = """
            SELECT c.id, c.name, c.phone, COUNT(t.id) AS visits
            FROM infrastructure_ticket t
            JOIN infrastructure_vehicle v ON v.id = t.vehicle_id
            JOIN infrastructure_client c ON c.id = v.client_id
        ...
        with connection.cursor() as cursor:
            ...
            rows = cursor.fetchall()
        return [
            {"client_id": r[0], "name": r[1], "phone": r[2], "visit_count": r[3]}
            for r in rows
        ]
