...
from .models import ParkingSpot # Para calcular la ocupación real

def reports_view(request):
    repo = DjangoReportRepository()
    
    # 1. Datos Financieros (Hoy, Semana, Mes)
    finanzas = repo.get_financial_summary()
    comparativo_dias = repo.get_revenue_by_day_of_week()
    
    # 2. Estadísticas de tiempo y vehículos
    # Asegúrate de tener estos métodos en tu DjangoReportRepository
    stay_metrics = repo.get_stay_metrics() 
    vehicle_stats = repo.get_vehicle_type_stats()
    frequent_clients = repo.get_frequent_clients()
    
    # 3. Datos para la tabla de historial mensual
    monthly_income = repo.get_monthly_income()

    # 4. Cálculo de ocupación (Igual que en tu dashboard)
    total_spots = ParkingSpot.objects.count()
    occupied_count = ParkingSpot.objects.filter(status="OCCUPIED").count()
    ocupacion = int((occupied_count / total_spots) * 100) if total_spots > 0 else 0

    # 5. Otros cálculos rápidos
    promedio_diario_mes = finanzas['mes'] / 30 if finanzas['mes'] > 0 else 0
    # Evitar división por cero si hoy no hay tickets
    ...
    context = {
        ...
        "daily_income": {
            "total": finanzas['hoy'],
            "ticket_count": finanzas['tickets_hoy']
        },
        ...
        # Operación
        "stay_avg": stay_metrics.get('avg_time', 0),
        "stay_max": stay_metrics.get('max_time', 0),
        "occupancy_rate": ocupacion,
        "vehicle_stats": vehicle_stats,
        "frequent_clients": frequent_clients,
        
        # Metadata
        "meta_diaria": promedio_diario_mes,
        "hoy": date.today(),
    }
